...
securityRouter.get("/audit-log", authenticateToken, requireAdminAccess, async (req: Request, res: Response) => {
  try {
...
securityRouter.get("/alerts", authenticateToken, requireAdminAccess, async (req: Request, res: Response) => {
  try {
...
securityRouter.get("/stats", authenticateToken, requireAdminAccess, async (req: Request, res: Response) => {
  try {
...
securityRouter.get("/failed-attempts/:ip", authenticateToken, requireAdminAccess, async (req: Request, res: Response) => {
  try {
    const ipAddress = req.params.ip;
    const hours = parseInt(req.query.hours as string) || 24;
    
    const attempts = await securityAudit.getFailedAttemptsByIP(ipAddress, hours);
...
securityRouter.get("/dashboard", authenticateToken, requireAdminAccess, async (req: Request, res: Response) => {
  try {
