...
class AuthService {
  ...
  private async loadEmployees(): Promise<any[]> {
    try {
      const filePath = path.join(process.cwd(), 'public/data/sage-300-database.json');
  ...
  async login(credentials: LoginRequest): Promise<LoginResponse> {
    try {
      ...
      const user: AuthUser = {
        ...
        accessRoles: employee.AccessRoles || [this.mapRoleToSystemRole(employee.Role)]
      };
  ...
  async verifyToken(token: string): Promise<AuthUser | null> {
    try {
      ...
      // Get fresh user data
      const employees = await this.loadEmployees();
      const employee = employees.find(emp => emp.EmployeeID === decoded.employeeId);
  ...
  async changePassword(employeeId: string, currentPassword: string, newPassword: string): Promise<{ success: boolean; message: string }> {
    try {
      const employees = await this.loadEmployees();
