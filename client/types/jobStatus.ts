...
export const STATUS_TRANSITION_CONDITIONS: StatusTransitionCondition[] = [
  ...
  // Assigned -> Accepted
  {
    ...
    condition: (jobData, userRole) =>
      ...
      (jobData.assignedTechnician === userRole ||
        jobData.assistantTechnician === userRole),
  ...
  // In Progress -> Stopped
  {
    ...
    to: "stopped",
    condition: (jobData, userRole) =>
      ["Technician", "AssistantTechnician", "Coordinator", "Manager"].includes(
        userRole,
      ),
  ...
  // Tech Finished -> Sent Back
  {
    ...
    condition: (jobData, userRole) =>
      ["Coordinator", "Manager"].includes(userRole) &&
      (jobData.qualityCheckPassed === false ||
        jobData.customerSignOff === false),
  ...
  // Job Completed -> Sage Error
  {
    ...
    condition: (jobData, userRole) => jobData.sageIntegrationError === true,
