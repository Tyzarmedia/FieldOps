...
export default function TeamTrackingPage() {
  ...
  const loadTechnicianData = async () => {
    const mockTechnicians: Technician[] = [
      ...
      },
      {
        ...
        role: 'assistant',
        status: 'in_transit',
        location: { lat: -25.7545, lng: 28.1912, address: 'En route to Business Park' },
        currentJob: {
          id: 'JOB-002',
      ...
      },
      {
        ...
        role: 'technician',
        status: 'idle',
        location: { lat: -25.7489, lng: 28.1956, address: 'Office - Waiting for assignment' },
  ...
  const filteredTechnicians = selectedTeam === 'all' 
    ? technicians 
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-gray-600">
              Real-time visibility and control over team performance and location
            </p>
          </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{clockedInCount}/{technicians.length}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{activeJobsCount}</p>
              </div>
      ...
      <Tabs defaultValue="overview" className="w-full">
                ...
                <TableBody>
                  {teams.map((team) => (
                    <TableRow key={team.id} className="cursor-pointer hover:bg-gray-50"
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Progress value={team.productivityScore * 100} className="w-16" />
                          <span className="font-bold">{(team.productivityScore * 100).toFixed(0)}%</span>
                        </div>
                      </TableCell>
        ...
        <TabsContent value="map" className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-semibold">Live Team Location & Heatmap</h3>
            <div className="flex gap-2">
              ...
              <Button variant="outline" onClick={() => setMapView(mapView === 'standard' ? 'heatmap' : 'standard')}>
              ...
              <Card>
                <CardContent className="p-6">
                  <div className="h-96 bg-gradient-to-br from-blue-50 to-green-50 rounded border-2 border-dashed border-gray-300 flex flex-col items-center justify-center">
                    ...
                    <p className="text-lg font-medium text-gray-600 mb-2">Live GPS Tracking Map</p>
                    <p className="text-sm text-gray-500 text-center max-w-sm">
                      Real-time technician locations with {mapView} overlay showing team distribution across job sites
                    </p>
                        ...
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className={`w-2 h-2 rounded-full ${
                              tech.status === 'on_site' ? 'bg-green-500' : 
                              tech.status === 'in_transit' ? 'bg-blue-500' : 
                              tech.status === 'idle' ? 'bg-yellow-500' : 'bg-red-500'
                            }`}></div>
                          </div>
                        </div>
                        <div className="text-xs text-gray-600">
                          ...
                          {tech.currentJob && (
                            <div className="flex items-center gap-1">
                              ...
                              {tech.currentJob.name} ({tech.currentJob.timeOnJob}m)
                            </div>
                          )}
                        </div>
                      </div>
              ...
              <Table>
                ...
                <TableBody>
                  {technicians.map((tech) => {
                    const scheduled = new Date(`2025-01-25 ${tech.scheduledClockIn}`);
                    const actual = tech.clockInTime ? new Date(`2025-01-25 ${tech.clockInTime}`) : null;
                    const lateMinutes = actual && actual > scheduled 
                      ? Math.round((actual.getTime() - scheduled.getTime()) / (1000 * 60))
                      : tech.status === 'late' ? 45 : 0;

                    return (
                      <TableRow key={tech.id} className={
                        tech.status === 'late' ? 'bg-red-50' : 
                        lateMinutes > 0 ? 'bg-yellow-50' : ''
                      }>
                  ...
                  <div className="space-y-3">
                    ...
                    {tech.currentJob && (
                      <div className="bg-gray-50 p-3 rounded">
                        ...
                        <p className="text-sm text-gray-700">{tech.currentJob.name}</p>
                        <p className="text-xs text-gray-500">ID: {tech.currentJob.id}</p>
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <div>
                            <p className="text-xs text-gray-500">Time on Job</p>
                            <p className="text-sm font-medium">{tech.currentJob.timeOnJob}m</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Stock Usage</p>
                            <p className="text-sm font-medium">{tech.currentJob.stockUsed}/{tech.currentJob.stockAssigned}</p>
                          </div>
                        </div>
                      </div>
                    )}
        ...
        <TabsContent value="alerts" className="space-y-4">
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {alerts.map((alert) => (
                  <div key={alert.id} className={`p-4 border-l-4 rounded-r ${
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="font-medium">{alert.technician}</span>
                          <span className="text-sm text-gray-500">({alert.team})</span>
                          {getSeverityBadge(alert.severity)}
                        </div>
                        <p className="text-sm text-gray-700">{alert.message}</p>
                        <p className="text-xs text-gray-500 mt-1">Alert time: {alert.timestamp}</p>
                      </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="p-3 bg-gray-50 rounded">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    Coordinates: {selectedTechnician.location.lat}, {selectedTechnician.location.lng}
                  </div>
                </div>
              </div>
              ...
              {selectedTechnician.currentJob && (
                <div>
                  ...
                  <div className="p-3 bg-gray-50 rounded">
                    <div className="font-medium">{selectedTechnician.currentJob.name}</div>
                    <div className="text-sm text-gray-600">ID: {selectedTechnician.currentJob.id}</div>
                    <div className="grid grid-cols-2 gap-4 mt-2">
                      <div>
                        <span className="text-xs text-gray-500">Time on Job:</span>
                        <div className="font-medium">{selectedTechnician.currentJob.timeOnJob} minutes</div>
                      </div>
                      <div>
                        <span className="text-xs text-gray-500">Stock Usage:</span>
                        <div className="font-medium">{selectedTechnician.currentJob.stockUsed}/{selectedTechnician.currentJob.stockAssigned} items</div>
                      </div>
                    </div>
                  </div>
                </div>
