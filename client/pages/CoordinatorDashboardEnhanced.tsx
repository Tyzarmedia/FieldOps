        ...
        t.id === technicianId
          ? {
              ...
              timeSlots: t.timeSlots.map((slot) =>
                slot.hour >= timeSlot &&
                slot.hour <
                  timeSlot + Math.ceil(draggedJob.estimatedDuration / 60)
                  ? {
                      ...slot,
                      status: "busy",
                      jobId: draggedJob.id,
                  ...
                  {unassignedJobs.map((job) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-500 mt-1">
                        Duration: {job.estimatedDuration}m | Skill:{" "}
                        {job.skillRequired}
                      </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-12 gap-2 text-xs text-center font-medium">
                      ...
                      {Array.from({ length: 10 }, (_, i) => i + 8).map(
                        (hour) => (
                      ...
                      >
                        ...
                        {tech.timeSlots.map((slot) => (
                          <div
                            ...
                            onDrop={(e) =>
                              handleTechnicianDrop(tech.id, slot.hour, e)
                            }
        ...
        <TabsContent value="monitor" className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <Select
                value={progressView}
                onValueChange={(value: "kanban" | "map" | "timeline") =>
                  ...
                  {kanbanColumns.in_progress.map((job) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-blue-600 mt-1">
                        Assigned to:{" "}
                        {technicians.find((t) => t.id === job.assignedTo)?.name}
                      </div>
                      <div className="flex gap-1 mt-2">
                        {getPriorityBadge(job.priority)}
                        {getSLABadge(job.slaStatus)}
                      ...
                      <div className="flex gap-1 mt-2">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => updateJobStatus(job.id, "delayed")}
                        >
              ...
              <CardContent>
                <div className="h-96 bg-gradient-to-br from-blue-50 to-green-50 rounded border-2 border-dashed border-gray-300 flex flex-col items-center justify-center">
                  ...
                  <p className="text-sm text-gray-500 text-center max-w-sm">
                    Real-time GPS tracking of field technicians with job status
                    indicators
                  </p>
                </div>
              </CardContent>
                  ...
                  {technicians.map((tech) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Progress
                          value={tech.availability === "busy" ? 80 : 20}
                          className="w-16"
                        />
                        <span className="text-xs">
                          {
                            tech.timeSlots.filter((s) => s.status === "busy")
                              .length
                          }
                          /10h
                        </span>
        ...
        <TabsContent value="kpi" className="space-y-4">
          <div className="mb-4">
            <h3 className="text-lg font-semibold mb-2">
              Manager-Customized KPI Dashboard
            </h3>
            <p className="text-sm text-gray-600">
              View the KPI dashboard configured by your manager. This dashboard
              is automatically updated with the latest metrics.
            </p>
          </div>
