...
const testJobAPI = async () => {
  try {
    ...
    const techJobsRes = await fetch('http://localhost:8080/api/job-mgmt/jobs/technician/tech001');
    const techJobs = await techJobsRes.json();
    console.log('Tech001 jobs:', JSON.stringify(techJobs, null, 2));
    
    // Test job stats for tech001
    const statsRes = await fetch('http://localhost:8080/api/job-mgmt/jobs/stats/tech001');
